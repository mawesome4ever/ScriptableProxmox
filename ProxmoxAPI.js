...
//     log("type: "+roughSizeOfObject(string));
    if ( string ) {
      return roughSizeOfObject(string);
    }
  ...
  return lengthInUtf8Bytes(string);
}
...
/** @type {string} */
  ...
  constructor(hostname, port = 8006) {
    this.#debug = importModule("Debug");
    this.#hostname = hostname;
    this.#port = port;
    this.#log = this.#debug('proxmox-ve:debug').log;
    this.#error = this.#debug('proxmox-ve:error').log;
...
//     log("params size: "+Object.entries(parameters).length - 1);
    for (const [key, value] of Object.entries(parameters)) {
//       log("ind: "+ind+", size: "+ Object.entries(parameters).length);
      if (value != null && value != undefined) {
        if (typeof value == "boolean") {
          tmpParameters[key] = value ? 1 : 0;
          urlencoded += key+"="+(value ? "true" : "false")+"&";
        } else {
    ...
    const jsonstr = JSON.stringify(urlParams);
    if (method == 'GET' || method == 'DELETE') {
      if (urlParams.length > 0) { url += '?' + encodeURIComponent(urlencoded); }
    } else {
      body = urlencoded;
      headers = {
    ...
    return new Promise(async (resolve, reject) => {
      let urlRequest = options.host + ":" + options.port + options.path; 
//       log("REQUEST URL: "+ url);
//       log("headers: "+JSON.stringify(headers));
//       log("body: "+body);
      try{
        let req = new Request(urlRequest);
        req.body = body;
        req.headers = headers;
        req.method = method;
  ...
  login(username, password, realm = 'pam', otp = null) {
    ...
    return new Promise((resolve, reject) => {
      this.create("/access/ticket", { password: password, username: username, realm: realm, otp: otp })
        ...
        }).catch((error) => { reject("[login]"+error) });
    });
  }
...
module.exports.PveClientBase = PveClientBase;
